...
export default function EditCourtScreen() {
  ...
  const fetchCourt = async () => {
    ...
    try {
      ...
      const foundCourt = courts.find((c: Court) => c.court_id === id);
      if (foundCourt) {
        setCourt(foundCourt);
        setName(foundCourt.name);
        setSelectedSport(foundCourt.sport);
        setIsIndoor(foundCourt.is_indoor || false);
        setNotes(foundCourt.notes || '');
        setSelectedHours(foundCourt.available_hours || []);
      } else {
  ...
  const handleSubmit = async () => {
    ...
    setIsSaving(true);
    try {
      await apiClient.updateCourt(id!, {
        name,
        sport: selectedSport,
        is_indoor: isIndoor,
      ...
      Alert.alert(t('success'), 'Campo modificato con successo', [
        { text: 'OK', onPress: () => router.back() },
      ]);
    } catch (error: any) {
      Alert.alert('Errore', error.response?.data?.detail || 'Impossibile modificare il campo');
    } finally {
      setIsSaving(false);
    }
  };
